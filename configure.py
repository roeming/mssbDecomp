...
assert os.path.exists(c.DOL), "Error: Base binary not found"

# Check binaries are correct
dol_hash = c.get_file_sha1(c.DOL)
assert dol_hash == bytes.fromhex("8a7f951cd701a9db6ed13a0bd59456d7a44b6fbb"), \
...
n.rule(
    "ld",
    command = "$ld $ldflags -Map $map -T $ldscript $in -o $out",
    description = "LD $out",
)
...
n.build(
    ...
    implicit=[c.DOL_LDSCRIPT],
    implicit_outputs=c.DOL_MAP,
    variables={
        "map" : c.DOL_MAP,
        "ldscript" : c.DOL_LDSCRIPT
    }
)
